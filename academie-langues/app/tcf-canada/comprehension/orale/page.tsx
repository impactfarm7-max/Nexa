...
import { ArrowLeft, CheckCircle2, BookOpen, ChevronRight, Headphones, Clock, Target, TrendingUp, Lock, Star } from "lucide-react";
import { supabase } from "@/app/utils/supabase";
import { logClientActivity } from "@/app/utils/client-activity";
import { availableSeriesIdsCO } from "@/app/data/comprehension_orale";
import { useSimulationLimit } from "@/app/hooks/useSimulationLimit";
import { BRAND, STUDENT_TEXT } from "@/app/utils/brand";

const availableIds = new Set(availableSeriesIdsCO);
...
export default function ComprehensionOralePage() {
  ...
  useEffect(() => {
    logClientActivity("Ouverture comprehension orale", "Catalogue des series consulte");
          ...
          {filtered.map((serie) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (isLocked) return;
                  logClientActivity("Serie comprehension orale selectionnee", serie.label, { serie_id: serie.id });
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isLocked ? "bg-neutral-200" : isDone ? "bg-orange-500" : "bg-orange-100"}`}>
                    {isLocked
                  ...
                  <div>
                    <p className={`text-sm font-black leading-tight ${isLocked ? "text-neutral-400" : isDone ? "text-orange-900" : "text-neutral-900"}`}>{serie.label}</p>
                    <p className={`text-[11px] font-semibold ${isLocked ? "text-neutral-400" : isDone ? "text-orange-500" : "text-neutral-500"}`}>
                      {isLocked ? "Bientôt disponible" : isDone ? `${result.level} · ${result.score}/${result.total}` : `${TOTAL_QUESTIONS} questions`}
                    </p>
                  </div>
                </div>
                {!isLocked && (
                  <span className={`text-[11px] font-black flex items-center gap-0.5 ${isDone ? "text-orange-500 group-hover:text-orange-600" : "text-orange-600 group-hover:text-orange-700"}`}>
                    {isDone ? "Refaire" : "Commencer"} <ChevronRight className="w-3.5 h-3.5" />
                  </span>
                )}
              </button>
            );
