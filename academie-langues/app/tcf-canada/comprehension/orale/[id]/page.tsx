...
import { ArrowLeft, Headphones, Clock, HelpCircle, Zap, ChevronRight } from "lucide-react";
...
export default function ConsigneOralePage() {
  const router = useRouter();
  const params = useParams();
  const id = Number(params.id);

  useEffect(() => {
    logClientActivity("Ouverture serie comprehension orale", `Serie ${id}`, { serie_id: id });
  }, [id]);
    ...
    <div className="min-h-screen bg-[#FFFBF7] text-neutral-900 flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-neutral-200/60 shadow-sm">
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-orange-600">Compréhension de l'Oral</p>
        ...
        >
          ...
          <div className="flex flex-col items-center mb-8">
            <div className="w-20 h-20 rounded-[2rem] bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center shadow-xl shadow-orange-500/25 mb-5">
              <Headphones className="w-10 h-10 text-white stroke-[1.8]" />
            </div>
          ...
          <div className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-2xl px-5 py-4 mb-4">
            <div className="w-8 h-8 bg-orange-100 rounded-xl flex items-center justify-center shrink-0">
              <Headphones className="w-4 h-4 text-orange-600 stroke-[2]" />
            </div>
            <p className="text-xs font-semibold text-orange-800 leading-snug">
              Cette épreuve contient des fichiers audio qui seront joués automatiquement. Assurez-vous que votre son est activé.
            </p>
          </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-8">
            ...
            <ul className="text-xs text-amber-900 font-medium space-y-1.5 leading-relaxed">
              ...
              <li>Écoutez chaque document attentivement avant de répondre.</li>
              <li>Concentrez-vous sur les <span className="font-black">20 dernières questions</span>, car ce sont celles qui rapportent le plus de points.</li>
            </ul>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => {
                logClientActivity("Quiz comprehension orale lance", `Serie ${id}`, { serie_id: id });
                router.push(`/tcf-canada/comprehension/orale/${id}/quiz`);
              }}
