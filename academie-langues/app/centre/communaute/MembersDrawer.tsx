...
const ROLE_LABELS: Record<string, string> = {
  center_manager: "Directeur",
  campus_manager: "Dir. campus",
  staff: "Administratif",
  trainer: "Formateur",
  student: "Étudiant",
  admin: "Admin Nexa",
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex justify-end">
      <div className="w-full max-w-md bg-white h-full flex flex-col shadow-2xl">
        ...
        <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
          <div>
            <h3 className="font-black text-sm" style={{ color: BLUE }}>Membres</h3>
            <p className="text-[10px] text-neutral-400 mt-0.5">{roomName} · {members.length} membre{members.length > 1 ? "s" : ""}</p>
          </div>
        ...
        {showAdd && (
          <div className="px-5 py-3 border-b bg-orange-50/50 shrink-0">
            <p className="text-[10px] font-black uppercase text-neutral-400 mb-2">Ajouter un membre du centre</p>
            ...
            {!searchingCandidates && searchAdd.trim() && candidates.length === 0 && (
              <p className="text-[11px] text-neutral-400 italic py-2">Aucun résultat ou tous déjà membres.</p>
            )}
                ...
                <div key={c.id} className="flex items-center justify-between bg-white rounded-xl px-3 py-2 border">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[9px] text-neutral-400 truncate">{c.email} · {ROLE_LABELS[c.role] || c.role}</p>
                    </div>
                  </div>
                  <button
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <p className="text-center text-xs text-neutral-400 py-10">Aucun membre trouvé.</p>
          ) : (
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {m.room_role !== "member" && (
                          <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded bg-orange-100 text-orange-700 border border-orange-200 shrink-0">
                            {ROOM_ROLE_LABELS[m.room_role] || m.room_role}
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-neutral-400 truncate">{m.email}</p>
                      <span className="text-[9px] font-bold text-neutral-300">{ROLE_LABELS[m.role] || m.role}</span>
                    </div>
                      ...
                      <div className="shrink-0">
                        {confirmRemove === m.user_id ? (
                          <div className="flex items-center gap-1">
                            ...
                            >
                              {removing === m.user_id ? <Loader2 size={10} className="animate-spin" /> : "Confirmer"}
                            </button>
                            <button onClick={() => setConfirmRemove(null)} className="h-7 px-2 rounded-lg text-[9px] font-bold bg-neutral-100">Non</button>
                          </div>
                        ) : (
                          <button
        ...
        <div className="px-5 py-3 border-t bg-neutral-50 shrink-0">
          <div className="flex items-center justify-between text-[10px] font-bold text-neutral-400">
            <span>{members.filter(m => ["admin", "center_manager", "campus_manager", "staff", "trainer"].includes(m.role)).length} staff</span>
            <span>{members.filter(m => m.role === "student").length} étudiants</span>
