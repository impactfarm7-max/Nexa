...
export async function downloadLessonContentPdf(opts: {
  ...
  try {
    ...
    const orgName = (opts.orgName || "").trim() || "Académie";

    let y = marginTop;
    let pageIndex = 1;

    const drawPageChrome = () => {
      // Fine accent line at top
    ...
    const titleLines = doc.splitTextToSize(opts.title || "Leçon", contentWidth);
    ...
    if (body) {
      ...
      doc.setTextColor(muted.r, muted.g, muted.b);
      doc.text("Aucun contenu texte pour cette leçon.", marginX, y);
    }
    ...
    return { ok: false, error: e instanceof Error ? e.message : "Échec du téléchargement." };
  }
}
...
export async function downloadLessonAttachment(opts: {
  ...
  if (opts.type === "video_link") {
    return { ok: false, error: "Ce lien s'ouvre à l'extérieur." };
  }

  const headers = await authHeaders();
  if (!headers) return { ok: false, error: "Connecte-toi pour télécharger." };

  try {
    const res = await fetch(`/api/lesson-media/${opts.mediaId}?download=1`, { headers });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      return { ok: false, error: (body as { error?: string }).error || "Téléchargement impossible." };
    }
