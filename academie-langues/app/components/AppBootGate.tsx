...
export default function AppBootGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Aperçu superadmin (« vue en tant que ») ou visite démo publique : pas de
    // session_token/PIN pour l'identité impersonnée — ce contrôle déclencherait
    // un signOut à tort.
    if (readSaViewAs() || readVisitMode()) return;
