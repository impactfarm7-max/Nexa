...
import { QuestionCO } from "@/app/data/comprehension_orale";
import { CheckCircle2, ChevronLeft, ChevronRight, Volume2, Headphones } from "lucide-react";
...
export default function EpreuveCO({ questions, onComplete, initialAnswers, onAnswerChange }: Props) {
  ...
  const isImageQuestion = q.options.every((o) => !o.text);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 md:p-5 xl:p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 xl:w-11 xl:h-11 bg-teal-50 border border-teal-100 rounded-xl flex items-center justify-center shrink-0">
            <Headphones size={18} className="text-teal-600" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] xl:text-xs font-black text-teal-600 uppercase tracking-[0.3em]">Compréhension Orale</p>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="px-5 py-3 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
            <span className={`inline-block px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border ${levelColor[q.level] ?? "bg-slate-50 text-slate-600 border-slate-200"}`}>
              Niveau {q.level}
            </span>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              Q{currentQ + 1}/{questions.length} · {q.points} pts
            </span>
          </div>

          {/* Audio player */}
          <div className="p-5 md:p-6 border-b border-slate-100 bg-gradient-to-br from-slate-50 to-white">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Instruction</p>
            <p className="text-sm text-slate-700 leading-relaxed mb-4">{q.instruction}</p>
            <div className="flex items-center gap-3 bg-white rounded-xl p-3 border border-slate-200 shadow-sm">
              <div className="w-10 h-10 bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl flex items-center justify-center shrink-0 shadow-sm shadow-teal-500/30">
            ...
            {q.image && (
              <img
                ...
                className="mt-4 rounded-xl max-h-56 object-contain mx-auto border border-slate-200"
              />
            )}
          </div>
          ...
          <div className="p-5 md:p-6">
            <p className="text-[9px] font-black text-orange-500 uppercase tracking-widest mb-3">Réponse</p>
            <div className={isImageQuestion ? "grid grid-cols-2 gap-3" : "space-y-2"}>
              {q.options.map((opt) => (
                <button
                  key={opt.label}
                  ...
                  className={`flex items-center gap-3 p-3.5 rounded-xl border-2 font-medium transition-all ${
                    isImageQuestion
                      ? "flex-col justify-center py-5 w-full"
                ...
                >
                  <span
                    className={`${isImageQuestion ? "w-10 h-10 text-sm" : "w-8 h-8 text-xs"} rounded-xl flex items-center justify-center font-black shrink-0 transition-colors ${
                      answers[currentQ] === opt.label
                  ...
                  {opt.text && <span className="flex-1">{opt.text}</span>}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {currentQ < questions.length - 1 ? (
          ...
          >
            <CheckCircle2 size={14} /> Valider CO
          </button>
        )}
      </div>
    </div>
