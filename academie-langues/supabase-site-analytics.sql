...
dates as (select generate_series(s.today-(s.days-1),s.today,interval '1 day')::date day from settings s),
daily as (select d.day,count(v.visitor_id)::integer visitors,coalesce(sum(v.page_views),0)::integer page_views,
 (select count(*)::integer from public.site_visitors sv where (sv.first_seen_at at time zone 'Africa/Douala')::date=d.day) new_visitors
 from dates d left join public.site_daily_visits v on v.visit_date=d.day group by d.day order by d.day),
...
select jsonb_build_object(
 'series',coalesce((select jsonb_agg(jsonb_build_object('date',day,'visitors',visitors,'newVisitors',new_visitors,'pageViews',page_views) order by day) from daily),'[]'::jsonb),
